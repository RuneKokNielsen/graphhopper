...
\end{equation}
The complexity shown in (\eq:tc_gaps_1) does not include adding the new edges to the graph, we need to do this can be done by iterating through the list of the nodes with the new parents.
\begin{equation}
\label{eq:tc_gaps_2}
\mathcal{O}\left(n \Delta^{s}\right)
\end{equation}
And because we need to calculate this for every graph, we combine the two time complexities (\ref{eq:tc_gaps_1}) and (\ref{eq:tc_gaps_2}), and multiply them with $N$, the final time complexity for calculating gaps can be seen in (\ref{eq:tc_gaps_3}).
\begin{equation}
\label{eq:tc_gaps_3}
\mathcal{O}\left(N n \Delta^{s + 1}\right)
\end{equation}
...
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
	\iffalse	\includegraphics[width=10cm]{imdb_labelled} \fi
...
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
	\iffalse	\includegraphics[width=10cm]{imdb_unlabelled} \fi
