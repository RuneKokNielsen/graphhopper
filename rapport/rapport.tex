...
\begin{figure}[H]
	...
	\begin{minipage}[t]{0.2\linewidth}
		...
		\end{tikzpicture}
	\end{minipage}
	\caption{\textit{Left: G. Right: $G_{a}$}}
...
GH outperformed WL on the airways data. Adding seems to negatively affect on performance but within standard deviation.\\
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
...
\begin{minipage}[c]{0.5\linewidth}
	...
	\begin{tabular}{c|c|c|c}
		...
		GH & G & 2 & \textbf{61.6800 (std. 2.0422)}\\
		WL & - & - & 59.48 (std. 1.7080)
	\end{tabular}
...
K matrices on gray scale\\
\begin{minipage}{0.6\linewidth}
	\hspace*{0.1in}
	\includegraphics[width=5cm]{reddit_trolls_k}
	\captionof{figure}{Reddit binary (unlabelled) K gray scale for GH.}
	\label{fig:reddit_binary_unlabelled_k_gh}
\end{minipage}
\begin{minipage}{0.6\linewidth}
In figure \ref{fig:reddit_binary_unlabelled_k_gh} we provide a gray scale image of the kernel matrix. As illustrated almost all inner products are in one of the two extremes (being either white or black) divided perfectly by their graph label showing that the 100\% accuracy result is indeed not a result of an error in SVM but of the fact that the kernel matrix is perfectly separable.
\end{minipage}

\paragraph{Collab (unlabelled):}
When inspecting the results from Collab (labelled), we see that WL outperforms GH. Gaps seems to have very small to none effect on the performance of GH. \\
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
...
On the labelled dataset GH almost reached the performance of WL when and achieved its best result with $s=5$. \\
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
...
\paragraph{SEMI:}
Due to the size of the graphs in SEMI we do not have time to properly cross-validate the choice of $\mu$ for the D$\times$G node kernel. Instead, we have chosen two values and compared the results. Interestingly, for $\mu=1$ we saw a significant improvement when using gaps while we saw a negative impact on performance for $\mu=0.1$ showing that the impact of gaps relies on, amongst other factors, the choice of node kernel parameters. In the end, GH was outperformed by WL.
\\
\begin{minipage}{0.6\linewidth}
	\hspace*{-1in}
...
We did not see a consistent improvement in accuracy when adding gaps to GH. In most cases we saw no meaningful change in performance in either direction. The most promising result was achieved on SEMI where adding gaps had a massive positive impact ($>10\%$) for a specific choice of $\mu$. However, actually computing the best result with GH would require cross-validating $\mu$ and $s$ together and at this time we can not say whether the best combination of these parameters includes a positive value of $s$.
...
\section{Synopsis}
\includepdf[pages=-]{synopsis/synopsis.pdf}
