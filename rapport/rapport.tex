...
Finding this transformation $\Phi$ is, however, a daunting task (let alone transforming all the data). Luckily, as proved by Mercer's theorem (see \cite{svm-graph-kernels}), for any symmetrical and positive semi-definite function $k(x_i,x_j)$ there will exist some transformation $\Phi$ such that $k(x_i,x_j)=\langle\Phi(x_i),\Phi(x_j)\rangle$. As a consequence, if we can define some function $k(x_i,x_j)$ with these properties then we can substitute this function (called a kernel function, or just kernel) into the optimization problem:
...
The task is then to find a kernel that manages to compare the features of the data in a meaningful way making them separable in the transformed space. The GraphHopper kernel attempts to do just that by analysing a combination of structure and node labels in graphs.
