...
We did not see a consistent improvement in accuracy when adding gaps to GH. In most cases we saw no meaningful improvement or some decrease in performance. Gaps seemed to have a negative impact on data based on chemical compounds while it did better on the social media data. The airways data scanned from images gave inconclusive results so further testing on data based on images would be interesting. The most promising result was achieved on the 3D data in SEMI where adding gaps had a massive positive impact ($>10\%$) for a specific choice of $\mu$. However, actually computing the best result with GH would require cross-validating $\mu$ and $s$ together and at this time we can not say whether the best combination of these parameters includes a positive value of $s$.
