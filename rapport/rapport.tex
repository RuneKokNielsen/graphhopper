...
\subsection{Gaps}
The following section is based on the description of GraphHopper with gaps on trees from \cite{gappy}
...
\end{figure}
(\ref{eq:gaps}) Needs to be changed, so it also works on graphs, and explained.
\begin{equation}
\label{eq:gaps}
(p^{(2)}(v), v), (p^{(3)}(v), v)...(p^{(s + 1)}(v), v)
\end{equation}
