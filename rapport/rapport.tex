...
\subsection{SVM}
Support vector machines (svm) is a binary classification technique which is used to predict classification of records in data, which consist of multiple dimensions. In figure \ref{fig:hyperplane} is it  shown how the svm will predict with a hyperplane which classification each of the records belong to, this example is based on linear svm, but we can also make a non-linear svm, when we are doing a non-linear svm it start to get complexed, because we still want to draw a linear hyperplane, and to do this we need to transform the data to a new dimension where these records can be split with a linear hyperplane. These transformations are unknown and can be hard to find, but with mercer's theorem it is show how we can use graph kernels instead of transforming, see \cite{svm-graph-kernels} for a better description of mercer's theorem.
\begin{figure}[H]
	\centering
	\includegraphics[width=10cm]{svm_plot}
	\caption{\textit{B1 is the hyperplane, which split the records into a classification.}}
	\label{fig:hyperplane}
\end{figure}

\subsection{Graph kernel}
Mercer's theorem says that if we have a symmetrical and positive semi-definite function, then we can show $k(x_i,x_j) = \langle\Phi(x_i),\Phi(x_j)\rangle$, this allows us to use a kernel function instead of the transformation. In (\ref{eq:kernel_function_phi}) is it shown how the transformation is connected to the svm, as seen in the formula it is attached to the optimization problem know as the dual lagrange problem.
\begin{equation}
\label{eq:kernel_function_phi}
L_D = \sum_{i=1}^{n}\lambda_i-\frac{1}{2}\sum_{i,j}\lambda_i\lambda_j y_iy_j\Phi(x_i)\cdot\Phi(x_j)
\end{equation}
And because we know that we can substitute the transformation with the kernel function, then we can rewrite the dual lagrange problem to use the kernel function instead, in (\ref{eq:kernel_function}) is it show how we can substitute the transformation with the kernel function.
\begin{equation}
...
Cross validation is a concept we use when testing our graph kernels, the basic concepts of cross validation is that we train our svm on a certain amount of the data, and then use the last amount of data to test on. We do this because if we would train our svm on the entire data, then it would know all the correct answers, and this would give an inflated accuracy result. But because we want the most accurate result, we do this multiple times, where we each time leave out a different amount of the data, often the data is split up in 10 sets, where we train on 9 of the sets and then test on the last set, we then change the test set each time, and when this is done 10 times, we would have tested across the entire data set, we then take the mean of all our results to get a estimate of our accuracy.
...
The project also contains a MATLAB script for cross-validating node kernel parameters and a C++ application for translating .mat files into the file format accepted by the standalone GraphHopper application.

\subsection{Possible extensions}
Given more time, one could consider implementing multiple extensions to the current application.

As mentioned earlier we have supplied a simple MATLAB script for cross-validating node kernel parameters. This script works by running the GraphHopper application repeatedly each time recomputing $M$ before computing $K$ using the new node kernel parameter. By implementing this functionality inside the application itself we could compute multiple $K$ matrices reusing the same $M$ matrix. On the same note we could consider cross-validating the choice of node kernel and the maximum gap size in the GraphHopper application.

Since the current application returns the $K$ matrix the natural extension of cross-validation would be to return a list of $K$ matrices each paired with the parameters they were computed with (which may then be classified and cross-validated). As this would potentially require a massive amount of memory it should be considered to implement the classification process in the application as well.
...
Basic specifications of the data sets used throughout our experiments are supplied in table \ref{table:data-sets}.
...
\begin{tabular}{c|c|c|c|c}
	Data set & Number of graphs & Avg. nodes & Max nodes & Attr. dimensionality\\
	\hline
...
\begin{table}[H]
	...
	\scalebox{0.6} {
		...
		Reddit binary (300-700) & Dirac & 83.6000 (std. 1.1255) & & & & &  
		\end{tabular}
	}
